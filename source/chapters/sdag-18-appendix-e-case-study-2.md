...
[^upstream-boundary]: See companion specification, Part I §5.3 and Part II-A §A.2 — the implemented `@trust_boundary` and `@trusted` decorators make tier transitions structurally explicit at function boundaries, and the shipped analyser carries the tier across assignments statically. The designed type-system enforcement layer, which would have made the tier part of the value's type and prevented the downstream `.get()` question from arising at all, was never built (Part I §10.3).
...
[^exception-register-variance]: The designed companion specification distinguished between deferred architectural fixes and genuine domain variance in its exception register, via per-exception elimination-path fields — a distinction that matters precisely when architectural decisions made at the specification layer produce downstream patterns that accumulate as exceptions rather than being fixed at their source. The register was never built; the as-built specification records the elimination-path ledger among the designed governance ideas worth revisiting (Part I §10.4).
