...
**Security standard extensions** addressing AI-generated code as a distinct trust category. For most organisations this means extending existing secure-development standards — internal coding and review policies, or frameworks such as the NIST SSDF (the US National Institute of Standards and Technology's Secure Software Development Framework) — with controls specific to agent-generated code. Organisations working under government frameworks have a head start: the ISM's 2025 updates, for example, provide strong foundations that require targeted extension rather than replacement. In all cases the shape is the same — not a new framework, but new controls within the existing structure.
...
Within an enterprise, the CISO (chief information security officer) or security engineering function is the natural owner of these extensions; at sector level, industry bodies and security standards organisations are positioned to coordinate shared rule sets and procurement guidance.
