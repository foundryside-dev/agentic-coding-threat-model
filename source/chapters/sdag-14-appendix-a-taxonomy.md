...
| ID | Name | STRIDE | Failure Layer [A] | Type | Relation [B] | Risk | Detection | Portable Tool (Wardline) [C] |
|----------|----------------------|------------|----------------|----------------|---------------|----------|-------------------|----------------|
| ACF-S1 | Fabricated Default | S | Training bias | Code Pattern | Agent-specific | High | Partial | Not covered (bespoke only) |
| ACF-S2 | Spurious Field Access | S | Training bias | Code Pattern | Agent-specific | High | Partial | Not covered (bespoke only) |
| ACF-S3 | Structural Identity Spoofing | S [D] | Training bias | Code Pattern | Agent-specific | High | Partial | Not covered (bespoke only) |
| ACF-T1 | Authority Tier Conflation | T | Training bias | Code Pattern | Agent-specific | Critical | None [E] | Covered |
| ACF-T2 | Silent Coercion | T | Training bias | Code Pattern | Agent-specific | Medium | Partial | Not covered (bespoke only) |
| ACF-T3 | Unstructured Signal Parsing | T | Training bias | Code Pattern | Agent-specific | High | Partial | Not covered |
| ACF-R1 | Audit Trail Destruction | R | Training bias | Code Pattern | Known class, agent-amplified | High | Partial | Partial |
| ACF-R2 | Partial Completion | R | Training bias | Code Pattern | Known class, agent-amplified | High | None | Partial |
| ACF-R3 | Verification Displacement | R | Context collapse | Code Pattern | Agent-specific | High | Partial (R3a) / None (R3b) [F] | Not covered |
| | — R3a: Verification Substitution | | | | | | | |
| | — R3b: Compensating Control Dependency | | | | | | | |
| ACF-R5 | Remediation-Induced Violation | R | Training bias | Code Pattern | Agent-specific | High | None | Indirect only |
| ACF-I1 | Verbose Error Response | I | Training bias | Code Pattern | Known class, agent-amplified | Medium | Partial | Not covered |
| ACF-D1 | Finding Flood | D | Process volume | Process Threat | Agent-specific | High | N/A | N/A (design constraint) |
| ACF-D2 | Review Capacity Exhaustion | D | Process volume | Process Threat | Agent-specific | High | N/A | N/A (design constraint) |
| ACF-E1 | Implicit Privilege Grant | E | Training bias | Code Pattern | Agent-specific | Critical | None | Covered |
| ACF-E2 | Unvalidated Delegation | E | Training bias | Code Pattern | Known class, agent-amplified | High | Partial | Partial |
| | **Provisional candidates below** — observed but not yet validated for core classification. Not compliance requirements. | | | | | | | |
| *ACF-S4* | *Type Annotation Erosion* | *S* | *Training bias* | *Code Pattern* | *Agent-specific* | *High* | *Partial* | *Not covered* |
| *ACF-S5* | *Type Structure Avoidance* | *S* | *Training bias* | *Code Pattern* | *Agent-specific* | *High* | *Partial* | *Not covered (bespoke only)* |
| *ACF-T4* | *Safety Guard Erosion* | *T* | *Training bias* | *Code Pattern* | *Agent-specific* | *Medium* | *None* | *Not covered* |
| *ACF-R4* | *Context Handover Assumption* | *R* | *Context collapse* | *Workflow Pattern* | *Agent-specific* | *Medium* | *Partial* | *Not covered* |
| *ACF-R6* | *Scope-Limited Triage* | *R* | *Context collapse* | *Workflow Pattern* | *Agent-specific* | *Medium* | *None* | *Not covered* |
...
**Detection approach:** Flag `.get()` and `getattr()` with defaults on objects whose type is annotated with an authority tier of Tier 1 (authoritative internal), Tier 2 (semantically validated), or Tier 3 (shape-validated). Requires authority tier annotations (not available in existing tools). Note: Semgrep custom rules can flag the structural pattern (`.get()` with non-None defaults) without provenance context, but with significantly lower precision — many legitimate uses of `.get()` with defaults exist, so the rule would require extensive per-project tuning or triaging. For internal state fabrication, flag `or ""`, `or 0.0`, `or 0`, and similar `or` fallbacks on fields that feed telemetry, metrics, or audit metadata. The designed companion specification mapped ACF-S1 to a member-access-with-fallback-default rule (WL-001), graded by declared context, with `or` fallbacks on Tier 1 data equally in scope. That rule was never built: the as-built reference implementation records ACF-S1 as *not covered* and names it the largest single gap between this paper's argument and the portable tool (companion specification, Part I §2.1, §10.1). The case study project's bespoke enforcement covers the pattern through a `.get()`-must-be-whitelisted rule (§8.3, §8.7); detection of ACF-S1 is therefore attributable to the bespoke layer, not the portable one.

The S1 pattern also extends to governance artefacts: an agent that fabricates a plausible-sounding rationale for a trust-escalation exception is substituting a plausible fabrication where genuine evidence should be required — the same mechanism as a fabricated default on a data field, applied to a governance decision rather than code. The designed companion specification addressed this through temporal separation, reviewer identity requirements, and recurrence tracking on exception rationales; none of that governance was built (companion specification, Part I §10.4). The case study project's signed exception register — judge triage plus an operator-held signing key — is the working control for this variant (§8.7).
...
**Detection approach:** An unconditional lint rule banning `hasattr()` catches all instances (the case study codebase in §8 enforces this). General-purpose linters do not flag `hasattr` because it is considered idiomatic Python. The designed companion specification treated `hasattr` as prohibited wherever structural guarantees are declared — an error in the trusted and validated states (named `INTEGRAL`, `ASSURED`, `GUARDED`, `UNKNOWN_ASSURED`, `UNKNOWN_GUARDED` in the as-built lattice), governable in the raw states (`EXTERNAL_RAW`, `UNKNOWN_RAW`, `MIXED_RAW`) — unlike `.get()` or `getattr()`, which are context-dependent: in high-stakes contexts there is no legitimate use of `hasattr` that cannot be expressed more safely as `isinstance()`, explicit `try`/`except AttributeError`, or an allowlist check. The designed WL-006 (runtime type-checking of internal data) would have provided secondary coverage. Neither rule was built: the as-built implementation records ACF-S3 as not covered directly — its nearest rule fires on tainted reflective attribute access, a sink rule rather than a structural-identity rule (companion specification, Part I §2.1, §10.1). The case study codebase's unconditional `hasattr` ban remains a bespoke rule. Detection is rated Partial because the rule is simple to implement but not present in any widely-deployed tool.
...
**Detection approach:** Taint analysis — trace the return values of functions marked `@external_boundary` (or matched by a known external call table) and flag if they reach trusted producers or data store operations without passing through a validation boundary marked `@trust_boundary(to_level="GUARDED")` or `@trust_boundary(to_level="ASSURED")`. This is the core capability of the companion specification's implemented taint engine (Part I §6.1), and ACF-T1 is the central case its trust-flow rules cover — untrusted data reaching a trusted producer, whether by return or by call argument.
...
**Detection approach:** Two tiers of coverage. *Default-based coercion* (`.get()` with non-None defaults, `or` chains with fallback values, ternary expressions with defaults) is partially detected: the case study's bespoke `.get()` rule covers that surface; `or` chains and ternary defaults are structurally detectable by custom Semgrep rules but with lower precision without authority tier annotations. *Type-casting coercion* (`float()` hiding precision loss, locale-dependent string operations, date parsing with assumed timezone) is not covered by any current rule set — no existing pattern rule targets type conversion on tier-classified data. The distinction from ACF-S1 is that T2 involves type conversion compounded with default substitution, not just default substitution alone. The as-built companion specification records ACF-T2 as not covered: default-based coercion was the territory of the unbuilt WL-001, and broader coercion was out of scope in the design and remains so (Part I §2.1).
...
**Detection approach:** Form (a): existing linters flag bare `except:` (no exception type) but not `except Exception:` (which is considered acceptable practice). Semantic detection requires understanding which operations are audit-critical — this is project-specific knowledge encoded in the trust topology (e.g., functions annotated as audit-write operations should not be inside broad exception handlers that continue on failure). Form (b): flag audit-critical call sites (e.g., functions annotated as `@audit_writer` or matching a known audit-operation list) where the call is not wrapped in a handler that raises a domain-specific exception type. The detection signature is the *absence* of a typed wrapper, not the *presence* of a catch block — a structurally different rule from form (a). Of the designed rules for form (a), two shipped: the as-built companion implementation covers broad exception handlers in trusted-tier functions (`PY-WL-103`) and silently swallowed exceptions in trusted-tier functions (`PY-WL-104`), rating ACF-R1 partially covered (companion specification, Part I §2.1). The designed WL-005 (audit writes inside broad handlers) and the audit-primacy annotations (`@audit_writer`, `@audit_critical`) that would have supplied its declaration mechanism were never built (Part I §10.1, §10.8). Form (b) was not covered by the designed rule set either — it requires a new detection category targeting untyped propagation from declared audit-critical operations.
...
**Detection approach:** No existing tool detects this in full — it requires understanding which operations form a logical transaction. The as-built companion implementation rates ACF-R2 partially covered, by the same two exception rules that cover ACF-R1 (`PY-WL-103`/`104`); the designed WL-005 and the operation-semantics annotations (`@atomic`, `@compensatable`) that would have addressed the transaction-context requirement directly were never built (companion specification, Part I §2.1, §10.8).

In the design, WL-005's relevance to R2 was distinct from its R1 role: it would fire on each individual audit-critical write inside a broad exception handler — for R1 catching the single audit failure being swallowed, for R2 providing a partial signal where multiple audit-critical writes share one handler. WL-005 alone would not have detected the atomicity gap; the designed `@atomic` and `@compensatable` annotations addressed the broader requirement that multiple state-modifying operations occur within a transaction context. None of this shipped. A semantic boundary enforcer could flag functions that contain multiple audit-write operations without a transaction context, but this requires project-specific annotation of which operations are audit-critical — precisely the declaration surface that was never built.
...
**Detection approach:** Existing scanners detect some cases (credential patterns, known sensitive variable names). Comprehensive detection requires understanding which variables contain sensitive information — a context-dependent judgement. AST-based rules can flag common patterns like `str(e)` in return values from exception handlers, but false positive rates vary by codebase. The designed companion specification mapped ACF-I1 to secret-handling and data-sensitivity annotation groups that would trace sensitive data through function bodies to detect logging, error-message, and unprotected-persistence exposure; neither group was built, and the as-built implementation records ACF-I1 as not covered — its log-format-injection rule touches the logging path but targets injection, not disclosure (companion specification, Part I §2.1, §10.8).
...
**Detection approach:** Taint analysis — the return value of an `@external_boundary` function is used as a predicate in an access control decision without passing through validation. Requires both boundary annotation and understanding of which operations are access-control-relevant. This taint-analysis capability is implemented in the companion specification's reference tool as tier-flow enforcement between declared boundaries (Part I §6.1); ACF-E1 is covered by the same trust-flow rules as ACF-T1 (Part I §2.1).
