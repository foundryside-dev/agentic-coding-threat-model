...
This section provides compact definitions for key terms used in the paper's core argument. Additional terms are defined where they are introduced; full definitions with cross-references appear in Appendix H (Glossary). The *companion specification* referred to throughout is a separate document — *Wardline: An As-Built Specification* — distributed with this paper and listed in the front matter. It is the as-built specification of a shipped reference implementation: a pure-Python static analyser, published on PyPI, extracted as the generic and portable component of the case study project's bespoke enforcement (§8.7). The original pre-implementation design (v0.2.0, archived) proposed considerably more than was built; the as-built document records what shipped, what was falsified by implementation, and what was designed but never built. It is included to demonstrate that the class of enforcement this paper calls for is technically feasible. It is a candidate approach, not the only possible one, and is not a prerequisite for the paper's conclusions — the paper's feasibility claim rests on the case study project's bespoke enforcement, operational since January 2026, of which the companion implementation covers a subset.
...
- **Observed patterns** are drawn from two sources: a simulation (§8.2, Appendix D) in which an agent prototyped an application from an underspecified brief, and approximately six months of direct experience with agentic development in a compliance-constrained environment (§8.3, Appendix E). The failure modes in the ACF taxonomy (Appendix A) were identified through observed agent behaviour, not theoretical analysis alone.
  ...
  The taxonomy originated from a recurring anomaly: agent-generated test suites that were partially missing or structurally broken in ways that did not correspond to any obvious coding error. Root-cause analysis traced the failures to context compression during long generation sessions (ACF-R2 and ACF-R3). That recurring pattern — *the same tool producing the same characteristic failure shape* — prompted the broader question of whether agentic code generation had other systematic failure modes invisible to standard review, which led to the STRIDE-based analysis and the taxonomy that follows. Quantitative signals (e.g., the violation rate reported in §8.3) are drawn from a single project and should be read as illustrative, not as population-level statistics.

  The taxonomy's core entries also have an enforcement lineage that answers "where did these categories come from?" concretely: they track the rules the longitudinal project's enforcement regime had to build, in the order it had to build them. `.get()` whitelisting came first and is ACF-S1; `hasattr` whitelisting is ACF-S3; exception tracking is ACF-R1; contract enforcement is ACF-T1/T2; untyped-dict bans are the provisional ACF-S5. The taxonomy was derived from enforcement experience, not from theory — each entry earned its place by forcing a rule into a working CI gate. The enforcement mechanism itself is not similarly new: §1.6 traces its lineage to Design by Contract, a control methodology that predates this paper's observations by nearly four decades.
...
This paper's proposed control — encoding institutional knowledge as machine-enforceable rules at a validation boundary — has a lineage that predates the LLM-code-bug literature surveyed above. Design by Contract, formalised by Meyer (1997), rests on a claim this paper makes independently about the assurance stack: a type system is necessary but not sufficient, because it can express that a value has the right shape — a well-formed classification literal, a non-empty string, a value within range — but not that it is the *correct* value for this record. Meyer's preconditions, postconditions, and invariants exist to check the second property; nothing in a type system checks it for you. The fabricated `"OFFICIAL"` in §2.3's worked example is exactly this failure: it is a well-formed classification, so every type checker, schema validator, and unit test passes it, and it is simply not the value this record is entitled to receive — a distinction the standard assurance stack was never built to make.

Design by Contract also treats a violated precondition as the caller's fault, not the routine's — a fault-attribution principle mirrored in this paper's own separation of ACF-S1 (a routine fabricates the value its caller failed to supply) from ACF-R1 (the evidence of that failure is then destroyed). Meyer argued against defensive programming inside a routine that has already checked its precondition, on the grounds that a duplicated check obscures which party is responsible for a fault — the same argument this paper makes about `.get()` defaults, made nearly forty years earlier. What is new is not the control but the threat source: Meyer's model assumed a human author trainable into contract discipline, whose assertions, once written, persisted across the project's lifetime. §2.4(a) argues that assumption does not hold for an agent — every session starts without the correction — which is a reason to treat mechanical enforcement as mandatory rather than optional: Meyer's own assertions could be switched off by build configuration, a latitude that enforcement at a commit-time validation boundary does not allow.

The guidance gap analysis in §6 examines governance frameworks (ISM, NIST SSDF, Essential Eight, OWASP) that claim authority over the software development lifecycle and finds them insufficient for agentic failure modes. The academic literature reviewed here is characterising the problem, not claiming to govern it. Both the governance frameworks and the research literature leave the same gap — institutional-context semantic correctness — which is the gap this paper's taxonomy, threat model, and control recommendations address.
