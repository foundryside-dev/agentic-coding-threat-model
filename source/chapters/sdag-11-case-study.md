...
The agent produced a complete, runnable FastAPI application — approximately 800 lines of code — in approximately 10 minutes. The application implements CSRF protection with `secrets.token_urlsafe()`, OTP hashing with a keyed SHA-256 scheme, constant-time comparison via `secrets.compare_digest()`, per-IP rate limiting, signed session cookies, Pydantic input validation, and structured audit logging. All automated checks pass. Both tests — there are only two — pass. The application starts, serves pages, and completes the full workflow without error.
...
The rules are enforced in CI by a project-specific AST pattern-matching tool with an allowlist-based exception regime. The enforcement model is not advisory — it is a gate. A pattern flagged by the enforcer either gets fixed by the agent or requires a human-authored exception with a rationale, an ownership tag, and an expiry date.[^reviewer-field] Legitimate uses of otherwise-restricted patterns go through; unconscious pattern completion from training data does not.

The enforcement described here and throughout §8.3 is the project's bespoke CI regime. The companion specification (Wardline) is a later, generic extraction from this experience covering the trust-flow and exception-discipline families; the project-specific institutional rules — `.get()` whitelisting, `hasattr` whitelisting, contract enforcement, untyped-dict bans — remain in the bespoke layer and were not generalised into the portable tool (§8.7).
...
**Which of this is the companion specification, and which is not.** The four-layer boundary described below is the case study project's bespoke enforcement architecture. Only Layer 1's scanner is the companion specification's reference implementation (Wardline), invoked with caller-granted trust and inertness detection. Everything else — the tier-model lint suite, the HMAC-signed exception register, the judge-at-signature-time workflow, the output oracles, and the dialect-parity invariants — is bespoke institutional machinery that was not generalised into the portable tool. The as-built companion specification takes an explicit "No governance" position for its own suppression files (Part I §7.4): no signatures, no reviewer identity, no output oracles. A reader who goes from this section to the portable tool expecting signed exceptions will not find them. The portable tool's answer to the same threat is mechanical — repository suppression files cannot clear the gate unless the caller grants them — and the signing regime described here sits on top of that as a project-level control.
...
[^taint-paths]: At the time of these observations the enforcement tool operated at the AST pattern-matching level — detecting defensive anti-patterns (`.get()` with defaults, `hasattr()` gates, broad `except` blocks) in authority-tier contexts — with full taint path analysis (tracking data flow across function boundaries) at that time a designed capability, not a deployed one. The project has since deployed a taint-analysis gate over declared trust boundaries — the taint engine is now specified as built in the companion specification, Part I §6.1 — and §8.7 describes the current machinery.

[^reviewer-field]: The current implementation uses category-level ownership tags (e.g., "architecture", "bugfix") rather than individual reviewer names. The designed companion specification proposed reviewer identity as part of an exception governance model; the as-built specification dropped the requirement entirely — its suppression records carry no ownership at all (companion specification, Part I §7.4). The bespoke regime's ownership tags are therefore stricter than the portable tool requires, not weaker than a standard demands.
