...
Habituation is a hazard specific to controls that ask for a judgment. A warning can be dismissed at no immediate cost, and the automation-bias literature above describes what repeated, low-cost dismissal does to a reviewer's calibration over time. A control that halts the pipeline is not offering a judgment to weigh — there is no dismissing it without also stopping the work, so the volume pressure that erodes attention to warnings has no equivalent purchase on it. This is the review-side expression of the argument in §2.4(a): a control that can be tolerated will, at sufficient volume, eventually be tolerated when it should not be, and only a control built to be intolerant of silence keeps producing the incident that correction depends on.
...
This means the traditional "warn first, enforce later" adoption strategy for security tooling is ineffective for agent-generated code. Agents require enforcement at the boundary before code enters the repository, not feedback over time, which depends on learning — the hard-failure end of the spectrum in §2.3 is this argument's limit case.
