...
The three versions also sit on an axis the paper returns to throughout. Under the contract discipline named in §1.6, `get_document_classification` carries an implicit precondition — that the caller supplies a record whose classification field is actually present — and a missing field is therefore the caller's violation to surface, not this function's to quietly repair. The fail-fast version surfaces it. The agent-generated version satisfies structural conformance and fails value conformance silently, in the sense §1.6 makes precise: nothing checked whether the field `"OFFICIAL"` stands in for was ever there. The table below shows the same substitution playing out across three domains, equally well-typed in every column — which is why no structural rule can separate the weather application's harmless default from the classification system's dangerous one.
...
The insistence on loudness through this paper is a consequence of that shift, not a separate preference for drama over grace. A crash is not merely the safer of two outcomes when a precondition is violated (§2.3) — it is the only one of the two that produces something to fix. A silent default leaves no incident, no diagnostic, and no case for the automated rule in §7.2 to encode. Because the agent retains no memory of a correction from one session to the next, the failure itself has to carry the memory: it must recur, identically, on every future encounter, until someone external to the agent turns it into a check the agent cannot bypass. Fail-fast is not simply the harm-avoiding choice on high-stakes paths — it is the mechanism by which a flaw becomes known, and therefore the only mechanism by which it becomes fixable.
