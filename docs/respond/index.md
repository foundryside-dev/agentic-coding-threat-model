...
**Scoping the enforcement perimeter — the first control.** Everything below applies to a *scoped subset* of the codebase, and the scoping decision is itself the first control. In most systems the overwhelming majority of code is not high-stakes: UI rendering, presentation formatting, service-continuity logic, and ordinary CRUD paths correctly use defensive programming, and applying fail-fast rules to them produces brittle software, developer friction, and a finding flood (ACF-D1) that discredits the enforcement layer. Applying semantic enforcement codebase-wide is a worse outcome than not applying it at all. Three converging tests identify a high-stakes path — a path is in scope if it passes any of them:
...
- **The consequence test.** If this code silently produced a wrong value, would the error propagate into a decision, a record of authority, or a downstream system before any human noticed? This is Q5 above, applied at scoping time rather than review time.
- **The failure-semantics test.** On this path, is a visible controlled failure preferable to continued operation on possibly-wrong data? If a crash would be worse than continuing — as it is on service-continuity and presentation paths — the path is out of scope, and defensive programming remains correct there.

A practical starting inventory, before any per-project analysis: authentication, authorisation, and account recovery; payment and entitlement handling; audit and compliance event recording; security classification and caveat handling; validation boundaries at external data ingress; and integrity-critical writes to authoritative stores. The output of scoping is not a judgement held in reviewers' heads but a **named, versioned artefact** — module lists, path globs, decorated functions, or annotated types — that the enforcement tool consumes and that changes only through review. Under-scoping is a recoverable error: the incident-response lifecycle below expands the perimeter whenever a violation is discovered on an ungoverned path. Everything outside the perimeter is explicitly exempt, and should stay exempt.

**The declaration burden is real and must be engineered down.** Three design choices keep the "annotation tax" proportionate. Declarations concentrate at *boundaries*, not throughout the codebase — the trust topology of a system is carried by a small number of ingress points, validation functions, and audit-critical operations, so annotating them is a bounded task even in a large codebase. Adoption is incremental: an unannotated module is outside the enforcement perimeter, not broken, so the perimeter can grow path-by-path from the highest-stakes modules. And agents can carry much of the burden — drafting candidate annotations from code structure for human confirmation is exactly the kind of mechanical-with-verification task agents perform well.
...
**Minimum verification properties.** Regardless of implementation approach, any enforcement tool proposed for high-assurance environments should be assessed against seven properties:
...
7. **Inertness detection** — the gate must distinguish "checked and clean" from "didn't look." A scan that loaded zero rules, recognised zero declared boundaries, or matched zero files must fail, not pass. A green result from an inert gate is worse than no gate at all, because every layer of governance above it — review posture, control law status, accreditation evidence — inherits the false signal. This is not hypothetical: the [longitudinal case study](case-study.md) ran one of its lint gates with zero rules loaded, exiting green, for months before the misconfiguration was noticed.

**Bootstrapping the golden corpus.** The corpus requirement has an apparent chicken-and-egg problem — it is meant to verify the tool that detects the failures, but the obvious source of known-bad specimens is the incidents the tool exists to prevent. Three sources break the circle without waiting for incidents:

- **Taxonomy-seeded specimens.** Every [ACF entry](../understand/taxonomy.md) carries paired examples — the agent-typical pattern and the correct alternative. Transcribed into the project's own language and domain vocabulary, these yield an initial known-bad/known-good set before the tool has ever run.
- **Synthetic failure injection.** Take known-good code from the project's own high-stakes paths and mechanically inject the anti-patterns — replace a direct field access with `.get()` and a default, wrap an audit write in a broad handler that logs and continues, delete the validation call between an external source and an internal sink. This is mutation testing applied to semantic rules, exercising the tool on the project's actual idioms. Agents generate the mutants cheaply; humans verify the labels.
- **Adjudicated-finding harvesting.** Once the tool runs, every adjudicated finding becomes a specimen: confirmed true positives join the known-bad set, approved exceptions join the known-good set with their rationale attached. The exception register and the golden corpus become two views of the same adjudication history.

Labels in all three sources must be human-verified — the corpus is the human-verified anchor that terminates the tool-verification recursion, and it loses that status if agents label their own specimens. For the same reason, the corpus should sit behind protected ownership, so that an agent cannot resolve a failing verification run by editing the expectations.
...
**The false-positive economics differ by who absorbs the finding.** Twenty years of SAST practice has been shaped by human tolerance — rules weakened, precision prioritised over recall, queues tuned to what developers will accept before they disable the tool. That calculus changes when the author absorbing the finding is an agent: an agent confronted with a red gate does not fatigue, resent the rule, or lobby to have it removed — it iterates until the gate passes. Organisations can therefore afford *stricter and less precise* rules on agent-authored changes than they would ever impose on human developers, prioritising recall over precision, because the cost of a false positive is agent iterations rather than human goodwill. The constraint relocates rather than disappearing: the human-facing exception path must stay scarce and adjudicated (a gate whose exceptions are cheap is not a gate), imprecise rules still teach the agent to satisfy the rule's letter rather than its intent, and the humans who adjudicate escalated exceptions remain subject to habituation — which is why proposed exceptions should be triaged before they reach a human decision.
...
**Automated pattern-wide remediation.** The controls above are detective and preventive. A third technical control category deserves formal status: using the same agentic velocity that creates the correlated-defect exposure to clear it, once a detection rule exists. Fifty instances of the same defensive anti-pattern across a codebase are one systematic defect, not fifty independent bugs — and once the pattern is encoded as a rule, remediation cost decouples from instance count. The lifecycle is: (1) a human recognises the failure shape and understands why it is wrong; (2) the understanding is encoded as a detection rule precise enough to act on; (3) the rule sweeps the codebase and finds every instance; (4) agents remediate the instances in parallel, with walltime largely independent of file count; (5) the rule joins the CI gate and prevents recurrence. Steps 1–2 are irreducibly human — the scarce resource is the detection rule, not the ability to push fixes. Two caveats keep the control honest. Remediation commits require *equal or higher* review scrutiny than new code, not less: ACF-R5 documents agents introducing a different violation inside the fix itself, precisely where reviewer attention is anchored on the original finding. And efficient targeting depends on provenance tracking — with provenance the sweep can prioritise agent-generated regions; without it, the sweep must cover everything.

**Baseline-and-ratchet enforcement for unpatternable rules.** Pattern-wide remediation presupposes that the failure shape can be encoded as a rule. Some invariants have trust domain, not syntax, as their subject — the same tokens are mandatory at one boundary and forbidden one function up — so any syntactic rule carries both false positives and false negatives *by construction*. For those invariants, freeze the corpus instead of matching the pattern: enumerate every current site of the construct, grant narrow structural amnesties for provably safe shapes, require a named adjudication entry for everything else, and fail the gate on drift in any direction — new sites, vanished entries, or changed occurrence counts. The machine enforces the property it *can* check ("no unadjudicated change") while humans adjudicate the property it cannot (whether each site is semantically legitimate), site by site, over time. Four design details carry the assurance value:

- **The gate must declare its own epistemic status.** A baseline seeded with unadjudicated entries must say so in its own header and failure messages, so green reads as "nothing new landed" rather than as a claim of correctness — a third epistemic state between "checked and clean" and "didn't look."
- **Key on occurrence counts, not merely site presence.** A working example caught its own recall gap when entry keys collapsed repeated occurrences within one function, leaving 29% of sites invisible until the distribution was measured.
...
- **Prefer a hole you can enumerate over one you have to trust.** A structural amnesty widened to cover nested variants creates a permanently invisible class; a handful of named entries costs nothing and stays countable. And because the adjudication ledger is itself authoritative data, its loader must fail loudly on any malformation rather than skipping a bad entry — the fail-fast-on-integrity rule the regime exists to impose, applied to the regime's own artefact.

This is deliberately proportionate: no signatures, no authority-tier coupling, no new tooling category. Not every semantic control needs the full enforcement stack.
...
Step 4 is the learning step that closes the loop. The execution mechanics of steps 1–3 at scale — parallel agent remediation once the rule exists — are the automated pattern-wide remediation control described under Technical controls above.
