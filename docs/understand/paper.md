...
The tooling around this limitation is evolving quickly — persistent memory stores, project-rule files, larger context windows, and checkpointing features are arriving faster than governance cycles can track, and specific failure modes (particularly the context-collapse layer described in (f) and (h) below) may attenuate as they mature. Two reasons the structural argument survives that evolution: first, memory stores and rule files change *where the rule lives*, not *what kind of thing it is* — the encoded correction is still an explicit declaration competing for salience against training priors, not internalised judgment from which the agent can generalise. Second, the training-distribution bias (§2.5) is untouched by session-level memory and, on the trajectory described there, may deepen. Controls should therefore be designed against the underlying condition — context-bounded generation from biased priors — rather than calibrated to the symptom rate of any particular tool generation.
...
The labels in all three sources must be human-verified — the corpus is the human-verified anchor that terminates the tool-verification recursion discussed in §9.8, and it loses that status if agents label their own specimens. For the same reason, the corpus should be under protected ownership (the companion specification's governance model places it behind CODEOWNERS-style review), so that an agent cannot resolve a failing verification run by editing the expectations.
...
### 8.7 Case study postscript: the validation boundary as built

*Added in v0.2.0. This subsection describes the longitudinal project's enforcement machinery as it stands at the time of writing — approximately five months after the observations in §8.3 and Appendix E were captured. It is included because the machinery now operationalises several of this paper's proposals as an executable CI pipeline, and because the failures encountered while building it are themselves evidence. The account is de-identified, consistent with §8's framing; the mechanisms, not the project, are the point.*

At the time of the earlier observations, the project's enforcement regime was a single AST pattern-matching gate with a YAML allowlist (§8.3). It has since grown into a four-layer validation boundary, each layer enforcing *meaning* at a different surface.

**Layer 1 — a semantic taint gate over declared trust boundaries.** The project's external ingress points are declared in code with decorator markers carrying per-boundary metadata (authority tier, source parameter, invariant). A small pack generator maps the marker functions themselves onto the trust grammar of a purpose-built taint-analysis scanner — an implementation of the declared-boundary approach the companion specification describes — seeding the taint state at each declared boundary; the richer per-boundary metadata is enforced separately by the project's own lint suite (Layer 2). The scanner performs genuine inter-procedural taint tracking — function- and variable-level propagation over an inter-module call graph — not AST pattern matching alone. Findings land at the *boundary*, not the sink: the fix discipline forces parsing and validation where untrusted data enters, which is where meaning is assigned. Two properties of the gate's invocation elevate this from linter to control. First, the *caller* grants the trust pack — the scanner refuses a repository's custom trust declarations unless the invoking pipeline explicitly authorises them, so repository configuration cannot quietly vouch for its own trust story. Second, the invocation enables inertness detection: a scan that recognised zero declared boundaries exits red rather than green, making "checked and clean" structurally distinguishable from "didn't look" (§7.2, verification property 7). The project pins this invocation — grant flags and inertness flag included — with a test over its own agent-instructions file, so the gate's posture cannot drift silently.

**Layer 2 — tier-model lints with judged, cryptographically signed exceptions.** A repository-local lint suite enforces the project's tier model (nominally type whatever the system itself owns; parse everything it does not; never accept a structural check as a security control). The last rule has a production incident behind it: a guard built on structural typing — a runtime-checkable protocol — passed impostor objects while rejecting the real vendor object, the ACF-S3 failure shape encountered live; in the project's own decision record, approximately 33,000 unit tests passed and one live HTTP call found it. The remediation campaign that followed that incident — including a stale-mandate near-miss and a corpus-ratchet control — is documented as Appendix E.8. Every suppression is a judged exception carrying owner, mandatory reason, and expiry (permanent exceptions are a separately counted, ratcheted category), sealed with an HMAC signature whose key only the human operator holds. Agents stage exception candidates key-free; only the operator can sign; and the signing step re-derives each finding from the live tree, so a staged claim that no longer matches reality fails at signature time. An exemption therefore carries provenance and cannot be minted by the code — or the agent — asking for it. This is the structural answer to the incident in Appendix E.2, where an agent resolved a tool conflict by writing itself a permanent allowlist entry: that pathway no longer exists. It also removes the self-authorisation route by which a remediation "fix" can silence the finding it was meant to resolve (ACF-R5).

**The judge in the loop.** Between the agent's proposal and the operator's signature sits an automated judge: a prompted model evaluates each flagged finding against the declared rule and produces a per-finding accept/block verdict with a bounded, prescribed-form rationale. The custody boundary is explicit in the tooling's own documentation — the judge signs nothing. The signing tool re-runs the judge on every substantive entry it is asked to sign (purely mechanical actions, such as key rotation, are exempt) and signs only what the judge accepts — re-judging at signature time is what prevents laundering a stale verdict over code that has since changed — while the agent-side judging run is a pre-screen that makes the operator's run cheap, not a substitute for it. A blocked verdict cannot enter the exception register through the normal path at all; the operator can override, but the override is recorded alongside the model's actual verdict, and the override rate is itself a tracked governance signal. The division of labour is deliberate: the judge absorbs the finding flood (ACF-D1) so that human attention lands only on ambiguous or high-stakes exceptions — before habituation can convert exception review into rubber-stamping (§4.2) — while the human, holding the key, remains the sole authority. Judging a *specific flagged violation against a specific declared rule* is also a far narrower task than not producing the pattern in the first place, which is why a model that reliably generates ACF-S1 can still usefully triage it. This is the §9.11 position — agent self-checking as load-bearing filter, unacceptable authority — in production form.

**Layer 3 — declared output oracles.** A scenario corpus pins content digests over the *semantic projections* of pipeline runs — a hash over what a pipeline's output means, not merely whether the pipeline executes — double-locked by a registry digest over the corpus itself. Any change to output meaning, even a single string inside a merge context, fails CI until a human consciously rotates the oracle and records why. The rotation discipline requires isolating the changed value and reproducing the prior digest byte-for-byte before the change may be accepted as mechanical. Conventional tests assert behaviour; the oracle asserts meaning — converting silent semantic drift (the ACF-T2 and ACF-R3 family, including tests quietly rewritten around changed behaviour) into a hard, human-adjudicated CI failure.

**Layer 4 — write-boundary invariants with dialect parity.** Where the platform itself is semantically permissive — the embedded database used in local development ignores column-length bounds that the production database engine enforces — the contract is encoded in code at the write boundary, with the bound derived from the schema so the two cannot drift, and verified by a containerised test suite against the real production engine. The motivating incident: a fully green local suite of tens of thousands of tests, and a run-killing overflow that only production could see. This closes, from the application side, the "application validates but the schema permits" gap identified in Appendix C §C.5.

**The honest ledger.** Two of these layers exist because the gates' own operators were fooled first, and the incidents are as instructive as the machinery. The first is the vacuous green: the tier-model lint command had run in CI for months in a form that loaded zero rules and exited success — a green that would have certified any tree. Two successive acceptance rounds were signed off against that gate before the hole was found; the project's own review notes observe that it had reproduced, in its second gate, exactly the inertness hole its first gate had been built to close. When the misconfiguration was found, the first real run produced a baseline of several hundred findings — a baseline, not a regression — and the incident motivated inertness detection as a named verification property (§7.2). The second is the deliberately red gate: at the time of writing, the tier-model gate exits non-zero with a substantial corpus of findings awaiting judged signature. That is the designed posture — unauthorised merges stay blocked while the signing debt stays visible — rather than a green gate purchased by weakening the rules. The defence is not that the codebase is clean; it is not. The defence is that every gate fails closed, distinguishes "checked and clean" from "didn't look," and requires a recorded human decision to overrule — which is what separates a codebase full of *known* semantic findings from one full of unknown ones.

**The pipeline as a whole.** Agent generates code → the semantic gate fails → the agent proposes an exception with rationale → the judge triages the proposal → the operator signs, or refuses, with a key the agent never holds. This is §5.3's validation boundary as an executable pipeline, and each element answers a threat this paper names: inertness detection answers vacuous assurance; caller-granted trust packs answer configuration self-authorisation; signed exceptions answer allowlist-broadening and ACF-R5; judge triage answers ACF-D1/D2; output oracles answer semantic drift; dialect-parity invariants answer the data-layer permissiveness gap. It also demonstrates the asymmetry noted in §7.2 and §8.6 in sustained operation: the gates are stricter than most teams would impose on human developers, and the agents simply iterate until they pass.

**Caveats.** The same single-project limitations as §8.3 apply: one codebase, one operator, machinery too young for outcome statistics. The gate state is red by design, and the signing debt is real. This account is offered as evidence that the paper's proposed control class is implementable — and of the failure modes an implementer should expect on the way — not as evidence that the approach is sufficient.

### 8.8 Operational tests and replication protocol
...
[^taint-paths]: At the time of these observations the enforcement tool operated at the AST pattern-matching level — detecting defensive anti-patterns (`.get()` with defaults, `hasattr()` gates, broad `except` blocks) in authority-tier contexts — with full taint path analysis (tracking data flow across function boundaries) a capability of the companion specification's design, not the deployed tool. The project has since deployed a taint-analysis gate over declared trust boundaries; §8.7 describes the current machinery.
...
- Automated first-pass triage of findings and proposed exceptions — for example, a prompted model evaluating each flagged violation against the declared rule before it reaches a human — so that human attention is reserved for ambiguous or high-stakes cases. Advisory only, never the authoritative gate (§8.7, §9.11)
...
A further candidate observation has arisen from remediation-phase work: **stale mandate execution** — an agent faithfully executing a work instruction that has been silently superseded or withdrawn by a governance artefact (an architecture decision record, a policy change) the instruction does not reference. The hazard grows with agent compliance and is compounded by statelessness: every session re-trusts the stale instruction afresh. Appendix E.8 documents the observed near-miss; the observation is put forward through this mechanism as a workflow-pattern candidate in the Repudiation family, not as a settled entry.
...
**E.8** (added in v0.2.0) is a postscript from approximately five months later: the remediation phase of the structural-typing incident cited in §8.7, in which an agent session discovered that its remediation ticket had been reversed by its own governing decision record — and what a 601-site survey revealed about syntactic bans on semantic properties.
...
*Added in v0.2.0, approximately five months after the incidents above.* The examples in E.1–E.6 show semantic defects being *caught*. This postscript documents the other half of the lifecycle — the same project operating inside the mature response — and it closes a loop opened in §8.7: the structural-typing guard incident cited there (a runtime-checkable protocol that passed impostor objects while rejecting the real vendor object, with approximately 33,000 unit tests green — the ACF-S3 failure shape encountered live) produced a corrective decision record, and that record's aftermath is where this example begins.
...
The counterfactual is the hazard worth naming. Agents are unusually literal and compliant executors of work instructions (§8.6) — and the more faithful the execution, the more dangerous a silently superseded instruction becomes. A human assigned the ticket might have remembered the week-old decision or asked; an agent handed the ticket as its task frame has no reason to doubt it, and the statelessness property (§2.4(a)) compounds the exposure: the agent re-trusts the stale ticket afresh in every session, indefinitely. This failure mode is distinct from the taxonomy's existing entries — it is not training-distribution bias (no pattern was being reached for), not context collapse (the context was present and correct; the *instruction* was wrong), and not ACF-R4's handover loss (nothing was dropped between sessions). The candidate name is **stale mandate execution**: an agent faithfully executes a work instruction that has been superseded or withdrawn by a governance artefact the instruction does not reference. The defect is not in the code the agent writes — it is that the agent writes the wrong code correctly. The mitigation is the family this paper already proposes: machine-readable governance state that the task frame is forced to consult — work instructions that cite their governing decision records, and tooling that fails the task when a cited record has been superseded. This observation is put forward as a candidate through Appendix A's taxonomy extension mechanism (a workflow pattern in the Repudiation family), not as a settled entry.

**Form predicted defect in neither direction.** The session's survey of all 601 sites is the paper's central technical argument quantified on a natural experiment. All 8 production sites the ticket's criteria marked "unconditionally banned" were legitimate on inspection — four were module facades resolving from closed literal tables (the very "explicit table" the ticket asks authors to write), and one was *itself an anti-masquerading guard*: the campaign's criteria would have deleted a control the campaign existed to install (the ACF-T4 erosion shape, arriving as a directive rather than a refactor). Meanwhile, both genuine defect candidates found in the corpus sat in the class the survey rated "probably fine" — structurally invisible to the ticket's criteria. On this corpus, a syntactic ban had approximately zero precision and approximately zero recall for the semantic property it targeted. In the session's own words: the same three tokens are mandatory at a vendor boundary and forbidden one function up — a ban written as a syntactic form cannot express a rule whose subject is trust domain.

**What shipped: a corpus ratchet.** Because the rule's subject cannot be expressed syntactically, the deliverable was not a migration but a freeze: a per-site baseline naming every site in the corpus, three narrow structural amnesties for provably safe shapes, and errors on drift in any direction — new sites, vanished entries, and changed occurrence counts all fail the gate. Deliberately no signatures and no authority-tier coupling: a proportionate, bespoke control (§7.2 discusses the general pattern). The gate carries its own epistemic status: most entries were seeded *unadjudicated*, recorded in the baseline's header and in the test messages, so a green gate reads as "nothing new landed" rather than as a claim that the corpus is correct. That is a third epistemic state between "checked and clean" and "didn't look" (§7.2, verification property 7), and it converts the corpus into a visible, classified worklist rather than a silent debt.

The ratchet's own verification produced a finding worth recording: the first build keyed entries on (path, function, kind), collapsing repeated probes within a single function — leaving close to a third of all sites invisible to the ratchet, clustered on the vendor-admission boundary the governing decision record had just corrected. Occurrence counts closed the gap. The weakening had been documented honestly in two places; only *measuring the distribution* showed that it mattered — measured recall (§7.2, verification property 4) demonstrated against the enforcement tool itself, and the §9.8 tool-verification recursion handled in practice (mutation checks confirmed the gate bites: with detection disabled, a spread of tests across the suite fail).

**What was deliberately left.** The test-lane sites — the substantial majority of the corpus — were left untouched — not for lack of time, but because the available signal cannot adjudicate them: a green suite is the expected outcome of both a correct rewrite and a rewrite that neutered the test. That is §9.9's coverage-illusion argument applied prospectively, as a review-planning constraint — the session recorded two defects rather than fixing them, and routed the adjudication to lead review, because "recorded, not fixed" was the honest posture the evidence supported.

**The adjudication phase.** The gate's first hours of live operation added three findings to the record. A resource-contended run walked only about 70 per cent of the tree — and the gate failed closed: every unreached site surfaced as a stale baseline entry (hundreds of errors rather than a false green), demonstrating that the ratchet doubles as a scan-completeness oracle in a way that per-root coverage assertions alone do not. A proposal to widen a structural amnesty to cover a nested variant was declined on stated principle — *prefer a hole you can enumerate over one you have to trust* — a handful of named, countable entries over one permanent blind spot. And the honest unadjudicated default revealed its own cost: seeding had marked the two most settled sites in the entire corpus — the corrected vendor-admission boundary that the governing decision record itself cites, and a function whose docstring explicitly forbids reverting to the protocol check — identically to the overwhelming majority of sites nobody had ever reviewed. The default was honest about the *seeding process*, not about the *sites*; and the danger of leaving them so was not wasted review effort but recurrence: a future reviewer re-deriving "this probe looks unguarded" from scratch is precisely the path by which the withdrawn protocol check gets reinstated and the original P0 returns. Both sites were adjudicated with justifications that carry the incident history and an explicit do-not-re-litigate warning, so that a future actor touching either function meets the history before the code. That is the correction-persistence problem (§2.4(a)) answered at site granularity: since neither humans nor agents reliably carry rationale forward, the rationale is stored at the exact location where the next change will happen — adjudication understood not as clearing sites but as capturing *why a site is the way it is, at the moment someone knows*.

**Observations.** Two, briefly. First, this example is the positive mirror image of E.2 and E.4: there, operator challenge was needed to surface what the agent had missed; here, the agent's investigation surfaced what a less-investigative executor would have walked straight into — a reminder that the same literalism that makes agents dangerous as generators makes them powerful as instruments of governance and investigation once directed at the right question (§8.5). Second, a generalisability caveat: this is Stage 3 operation (§7.2), and it works because the project holds preconditions most organisations do not — a mature decision-record practice capable of naming and withdrawing a ticket's instructions, bespoke enforcement machinery, a context-loaded operator, and a culture of honest accounting. The example evidences the *destination*, not the transition; the Stage 1 floor remains the entry point, and this postscript should not be read as raising the cost of admission.
