...
The more dangerous threat is subtler. AI agents write code that follows patterns generally regarded as good practice — defensive, robust, convention-conforming — and apply those patterns uniformly, including in contexts where they are catastrophic. The code is not merely plausible; it is conventionally reasonable by the standards of the vast majority of software. The problem is that agents produce excellent work calibrated to the wrong context.
