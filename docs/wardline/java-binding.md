...
This section defines the Java language binding for the [Wardline classification framework](specification.md). It maps the 17 abstract annotation groups (defined in the [framework specification](specification.md)) to concrete Java annotations, defines the interface contract for conformant scanners, and documents the residual risks specific to Java's language and ecosystem characteristics.
...
The Java binding is the second wardline language binding, developed after the [Python binding](python-binding.md) with the benefit of knowing which design decisions were framework-dictated versus Python-specific compensations.
...
**What transferred.** The annotation vocabulary maps all 17 groups from the [parent specification](specification.md). The scanner uses the same two-pass analysis model (symbol collection, then rule evaluation). The severity matrix, governance model, manifest schema, SARIF output contract, and golden corpus specification are binding-independent.

**What did not transfer.** The [Python binding](python-binding.md) dedicates substantial effort to compensating for language weaknesses: optional type checking, dynamic dispatch blind spots, runtime-only construction discipline, absent ownership semantics. Java eliminates several of these structurally. The Java binding is shorter in some areas (runtime structural enforcement is brief because the language provides it) and longer in others (residual risks address Java-specific concerns — framework proxy blind spots, Lombok-generated code, JPA entity lifecycle — that have no Python equivalent).
...
The [parent specification](specification.md) defines language evaluation criteria for wardline bindings. This section assesses Java against those criteria, modelling how future binding authors should evaluate their target language.
...
### B.2.2 Where Java structurally exceeds Python
...
3. **Immutability by construction.** Java records are immutable by language guarantee. Every component is `final`, there are no setters, and the canonical constructor requires all components. The [Python binding's](python-binding.md) `AuthoritativeField` descriptor is unnecessary — records make this structurally impossible.

These structural advantages mean the Java binding's assurance ceiling is meaningfully higher than the Python binding's.

### B.2.3 Ecosystem tool coverage
...
Any tool that implements Wardline-Core rules for the Java regime MUST satisfy the following interface contract (extending the [parent specification's](specification.md) conformance criteria):
...
4. **SARIF output.** The tool MUST produce findings in SARIF v2.1.0 with the wardline-specific property bags defined in the [parent specification](specification.md). The Java regime requires the following mandatory property bag keys on each `result` object:

    | Key | Type | Description |
    |-----|------|-------------|
    | `wardline.rule` | string | Binding rule ID (e.g., `JV-WL-001`) |
    ...
    | `wardline.annotationGroups` | integer array | Annotation groups active on the enclosing method |
...
A tool that satisfies this contract and implements at least one JV-WL rule is a partial Wardline-Core tool. A tool that implements all eight binding rules (JV-WL-001 through JV-WL-008) with tier-aware severity grading is a complete Wardline-Core tool.

**Rule mapping.** The eight Java binding rules map one-to-one from the eight framework rules ([Part I §7](specification.md)). Unlike the [Python binding](python-binding.md) — which splits WL-001 into two rules because Python has two distinct access-with-fallback idioms (`dict.get()` and `getattr()`) — the Java binding maintains a single JV-WL-001 because Java's fallback-default patterns (`Optional.orElse()`, `Map.getOrDefault()`, `Map.computeIfAbsent()`) share a unified AST shape:
...
| JV-WL-006 | WL-006 | Runtime type-checking internal data (`instanceof` on sealed types — tier-dependent, suppressed at Tier 4) |
| JV-WL-007 | WL-007 | Validation boundary with no rejection path (structural verification) |
| JV-WL-008 | WL-008 | Semantic validation without prior shape validation (validation ordering) |

JV-WL-001 through JV-WL-004 are syntactic patterns detectable by per-file AST analysis (the Error Prone advisory path). JV-WL-005 through JV-WL-008 require semantic context — audit-path awareness, tier classification, structural verification, or validation ordering — and are implemented by the reference scanner.

Tools that detect wardline-relevant patterns without satisfying this contract — e.g., Error Prone rules that match `Optional.orElse()` calls without consuming the manifest — are **advisory tools**, not Wardline-Core tools. Advisory tools provide useful early-warning feedback but their findings are not governance-grade.
...
Where the [Python binding](python-binding.md) uses decorator stacking to compose wardline markers, the Java binding uses meta-annotation composition. The difference is architectural:
...
The following table maps each of the 17 abstract annotation groups (defined in the [framework specification](specification.md)) to their concrete Java annotations.
...
| | | `@AuditWriter` | `@Target(METHOD)` | Audit-sensitive write. AUDIT_TRAIL severity. Return tagged AUDIT_TRAIL. Must precede `@EmitsTelemetry`. Fallback paths that bypass the audit call produce a finding |
...
This section covers design decisions specific to the Java binding. Where the rationale follows established precedent from [Part II-A (Python binding)](python-binding.md), the treatment is compressed — readers seeking the full reasoning should consult the corresponding section in Part II-A, which develops the general principles at greater length.
...
**`@PreserveCause` — parallel to Python's `@preserve_cause`.** Both bindings enforce cause-chain preservation: Python's `@preserve_cause` requires `raise ... from ...` in `except` blocks ([Part II-A](python-binding.md)); Java's `@PreserveCause` requires the caught exception to be passed as the cause parameter. The Java binding emphasises this annotation because Java's checked exception system makes cause-chain loss a particularly common and consequential problem — a `throw new SomeException(message)` inside a `catch` block that drops the caught exception destroys diagnostic information.
...
**`@EmitsTelemetry` — Java-specific extension.** `@EmitsTelemetry` has no Python equivalent because Python's telemetry patterns (logging, StatsD, OpenTelemetry) are typically function calls within `@audit_writer` bodies, and the ordering constraint (audit must dominate telemetry) is enforced by the scanner's call-site analysis within the existing `@audit_writer` annotation. Java's annotation-driven framework ecosystem (Micrometer, OpenTelemetry Java agent) makes telemetry emission a method-level declaration rather than a body-level call, requiring an explicit annotation to express the ordering constraint.

**`@TemplateRenderer` — Java-specific extension.** `@TemplateRenderer` has no Python equivalent because Python template rendering (Jinja2, Django templates) is typically performed through function calls that the scanner analyses within validation boundary bodies. Java's annotation-driven template engines (Thymeleaf, Freemarker integrations in Spring MVC) bind template rendering to method-level annotations and return types, requiring an explicit annotation to enforce that template input has reached Tier 2 or higher and that parameterised APIs are used.

**Contradictory combination detection.** The scanner detects mutually exclusive annotation combinations as ERROR findings, paralleling the [Python binding's](python-binding.md) SCN-021. The Java binding's combinations mirror the Python binding's 29-entry table with annotation name translations. Key Java-specific differences: `@PreserveCause` + `@ExceptionBoundary` is contradictory (preserve-cause implies propagation; exception boundaries terminate); `@TemplateRenderer` + `@ExternalBoundary` is contradictory (template input must be T2+; external boundaries produce T4). The full contradictory combination table is derived from the [Python binding](python-binding.md) by substituting annotation names per the B.4.2 mapping table — implementers should consult both tables. Stacking `@ValidatesShape` + `@ValidatesSemantic` on the same method is contradictory — use `@ValidatesExternal` for combined T4→T2.
...
The Java matrix has 4 SUPPRESS cells (vs. the framework's 2), 24 UNCONDITIONAL cells (vs. 25), and a corpus minimum of 120 effective specimens (60 active cells × 2).
...
Java's runtime structural enforcement layer is **substantially thinner** than [Python's](python-binding.md) because the language provides most enforcement structurally:
...
In the [Python regime](python-binding.md), advisory feedback (ruff) requires a separate pre-commit step. In the Java regime, advisory feedback fires during `javac` — developers receive advisory and type-system feedback simultaneously during compilation, before any separate analysis step.
...
The following residual risks are specific to the Java language binding. The [parent specification](specification.md) documents binding-independent residual risks — particularly risk 12 (evasion surface trajectory), which applies to both bindings: as annotation coverage grows, coding-level risk falls but governance risk rises.
...
**Compensating controls:** Do not use JPA entities as T1 data models. Use records or immutable domain objects for T1 artefacts. Annotate repository methods with `@IntData` and `@RestorationBoundary`. Prefer DTO projection queries (`SELECT new PartnerDTO(...)`) or Spring Data interface projections over entity fetching for read-only paths. Teams with existing JPA-heavy codebases should consider a phased migration: annotate entity repository methods with `@RestorationBoundary` immediately (Phase 2), then migrate T1 entity classes to records or immutable domain objects as the codebase evolves (Phase 3+). The immediate annotation gives the scanner visibility into the restoration boundary; the structural migration eliminates the mutable-state risk.
...
**Governance tooling is cross-cutting, not a separate phase.** The `wardline-cli` tool (manifest validation, fingerprint baseline, SARIF aggregation, control-law reporting) enters at Phase 1 and remains through all subsequent phases. The [Python binding's](python-binding.md) five-phase model includes a dedicated Phase 5 ("full governance") because Python's governance tooling matures separately from its scanner. In the Java regime, `wardline-cli` is shared across bindings and integrates naturally at each phase boundary — it does not require a separate adoption step.

**Java version requirement.** Java 17+ is a hard minimum. Records, sealed classes, and pattern matching are essential. Projects on earlier versions should plan a Java 17 migration before adopting this binding. A Java 11 project receives more value from the [Python binding](python-binding.md) on its Python services than from a degraded Java binding.
...
The Java regime's scanner error handling follows the same principles as the [Python binding](python-binding.md), with Java-specific adaptations.
