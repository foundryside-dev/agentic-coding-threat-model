...
This section provides the Python-specific binding reference for the [Wardline framework specification](specification.md). It covers the annotation vocabulary, interface contract, enforcement mechanisms, and residual risks specific to Python. The parent specification ([Part I](specification.md)) governs; this binding implements.
...
Iteration 3 used seven specialist agent perspectives to refine the design. Binary taint tracking was rejected by all seven agents; the team replaced it with the two-dimensional model tracking trust classification and validation status as orthogonal dimensions — now formalised in the [parent specification](specification.md).
...
*This section is non-normative. It models how future binding authors should assess their language against the [framework specification's](specification.md) evaluation criteria.*
...
4. **SARIF output.** The tool MUST produce findings in SARIF v2.1.0 with the wardline-specific property bags defined in the [parent specification](specification.md). The Python regime requires the following mandatory property bag keys on each `result` object:
...
**Rule mapping.** The nine Python binding rules derive from the eight framework rules ([Part I](specification.md)) as follows. WL-001 splits into two binding rules because Python has two distinct access-with-fallback idioms (`dict.get()` and `getattr()`); all other framework rules map one-to-one with a numbering offset:
...
**Coding posture per tier.** The [parent specification's](specification.md) authority tier model implies distinct programming styles:
...
The 17 annotation groups are defined as language-agnostic semantic requirements in the [framework specification](specification.md). This table provides the Python-specific decorator syntax. Decorators set `_wardline_*` metadata attributes on the decorated callable; they do almost nothing at runtime.
...
| 17 | Restoration Boundaries | `@restoration_boundary(...)` | `restored_tier`: int; `institutional_provenance`: str (opt); `structural_evidence`: bool; `semantic_evidence`: bool (opt); `integrity_evidence`: str (opt) | Body must satisfy WL-007. Evidence must support claimed tier per the evidence matrix in the [framework specification](specification.md). Scanner demotes effective taint state when evidence is insufficient. |
...
The Python enforcement regime composes existing ecosystem tools with a reference implementation to achieve Wardline-Full conformance per the [framework specification](specification.md).
...
*This section is non-normative. Assessors evaluating a Python wardline deployment should review these risks alongside the framework-level residual risks in the [framework specification](specification.md), particularly risk 12 (evasion surface trajectory) — as annotation coverage grows, coding-level risk falls but governance risk rises, and the risks below should be read in that context.*
...
The distinction between alternate and direct law follows the [framework specification](specification.md): alternate means degraded but running; direct means no meaningful enforcement output. Changes to wardline policy artefacts MUST NOT proceed under direct-law bypass.
