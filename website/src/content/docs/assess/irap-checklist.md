...
This checklist supports IRAP assessors evaluating systems that may include AI-generated code. It provides structured assessment criteria for the semantic defect class described in the [discussion paper](../../pdf/threat-model-discussion-paper-community.pdf) — code that is syntactically correct, passes all automated checks, and does the wrong thing in the system's institutional context.
...
2. What AI coding tools are in use? (e.g., inline autocomplete, IDE-integrated agents, autonomous agents.) At what autonomy level? (See [Autonomy Self-Assessment](../autonomy-assessment/) for the spectrum.)
...
12. Has the organisation assessed its exposure against the [ACF Taxonomy](../../acf/) failure modes?
...
- [ ] **Reviewer training.** Are reviewers trained to recognise ACF failure modes — particularly the distinction between defensive programming (good practice in most contexts) and defensive anti-patterns (dangerous in high-stakes contexts where silent data corruption is worse than a crash)?
...
:::caution[Provisional candidates (not core taxonomy)]
...
- [Full Discussion Paper](../../pdf/threat-model-discussion-paper-community.pdf) — the complete threat model, gap analysis, and evidence base
- [ACF Taxonomy](../../acf/) — the fifteen core failure modes (plus five provisional candidates) with STRIDE mapping, code examples, detection characteristics, and taxonomy extension mechanism for community-submitted entries
- [CISO Assessment](../ciso-assessment/) — the control-gap assessment for CISOs and security advisors, with ISM mapping and remediation priorities
- [Autonomy Self-Assessment](../autonomy-assessment/) — the autonomy spectrum for AI coding tools, referenced in pre-assessment question 2
