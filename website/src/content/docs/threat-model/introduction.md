...
While the primary focus is Python, the threat dynamics extend to other languages where agents generate executable logic. The [SQL extension](../../appendices/sql-extension/) analyses agentic SQL generation, where risks are amplified by direct data-store access and the prevalence of citizen programmers producing queries outside formal SDLC channels.
...
The perimeter of software production is expanding. Agentic tooling changes not only how fast code is produced, but *who produces it* and whether the work enters a channel that the organisation recognises as software development.
...
Key terms used throughout the threat model follow. Additional terms are defined where they are introduced; full definitions with cross-references appear in the [Glossary](../../reference/glossary/). The companion document, *Wardline: An As-Built Specification*, is the as-built specification of a shipped reference implementation: a **pure-Python static analyser, published on PyPI**, extracted as the generic and portable component of the case study project's bespoke enforcement. The original pre-implementation design (v0.2.0, archived) proposed considerably more than was built; the as-built document records what shipped, what was falsified by implementation, and what was designed but never built. It demonstrates that this class of enforcement is technically feasible, but it is only a candidate approach and is not a prerequisite for the paper's conclusions. The feasibility claim rests on the case study project's bespoke enforcement, operational since January 2026, of which the shipped analyser covers a subset.
...
| **Agent deployment spectrum** | The range from full human development (Level 0) through chat-pasted fragments (Level 1) and IDE-integrated agents (Level 2) to CI-integrated autonomous agents (Level 3). The [autonomy self-assessment](../../assess/autonomy-assessment/) provides a self-assessment framework. |
...
| **Assurance stack** | The standard verification pipeline: linters, type checkers, SAST, DAST, unit tests, and peer review. This paper argues the assurance stack does not adequately detect semantic failure modes — see the [ACF taxonomy](../../acf/). |
...
| **Habituation effect** | Progressive erosion of reviewer vigilance as repeated exposure to correct-looking agent output reduces the perceived need for scrutiny — a manifestation of the "Shifting the Burden" archetype. See the [systems thinking primer](../../appendices/systems-thinking/). |
...
- **Observed patterns** are drawn from two sources: a simulation in which an agent prototyped an application from an underspecified brief, and approximately six months of direct experience with agentic development in a compliance-constrained environment. The failure modes in the [ACF taxonomy](../../acf/) were identified through observed agent behaviour, not theoretical analysis alone. The taxonomy originated from a recurring anomaly: agent-generated test suites that were partially missing or structurally broken in ways that did not correspond to any obvious coding error. Root-cause analysis traced the failures to context compression during long generation sessions. That recurring pattern prompted the broader question of whether agentic code generation had other systematic failure modes invisible to standard review, which led to the STRIDE-based analysis and the taxonomy that follows. Quantitative signals are drawn from a single project and should be read as illustrative, not as population-level statistics.

  The taxonomy also has a concrete enforcement lineage: its core entries track rules the longitudinal project's enforcement regime had to build, in the order it had to build them. `.get()` whitelisting became ACF-S1; `hasattr` whitelisting became ACF-S3; exception tracking became ACF-R1; contract enforcement became ACF-T1 and ACF-T2; and untyped-dictionary bans informed the provisional ACF-S5. The entries were derived from enforcement experience rather than theory — each earned its place by forcing a rule into a working CI gate.

- **Analytical inferences** extend observed patterns through structured reasoning. The [STRIDE mapping](../stride/), the [guidance gap analysis](../guidance-gap/), and the compounding-effect argument are analytical — they apply established frameworks to observed phenomena. The conclusions follow from the analysis, but the analysis rests on a narrow empirical base.
...
This paper's analysis draws on three traditions that may not be equally familiar to all readers. The [systems thinking primer](../../appendices/systems-thinking/) provides a fuller primer with worked examples.
...
The [guidance gap analysis](../guidance-gap/) examines governance frameworks (ISM, NIST SSDF, Essential Eight, OWASP) that claim authority over the software development lifecycle and finds them insufficient for agentic failure modes. The academic literature reviewed here is characterising the problem, not claiming to govern it. Both the governance frameworks and the research literature leave the same gap — institutional-context semantic correctness — which is the gap this paper's taxonomy, threat model, and control recommendations address.

## A note on provenance

This paper practices what it preaches. The threat model, STRIDE mapping, ISM gap analysis, and ACF taxonomy were developed through the same process the paper describes: a practitioner with direct observational access to the phenomena — approximately six months of daily agentic development on a compliance-constrained system — using prompted AI collaboration to adopt analytical frames outside their primary expertise. The intelligence analysis tradecraft reflects the author's professional background; the domain-specific security analysis reflects prompted polymorphic review of the kind described in the [response landscape](../response-landscape/).
...
- [The Threat Landscape](../threat-landscape/) — the intuitive threat model and why it is incomplete
- [Trust Boundaries](../trust-boundaries/) — the authority-tier model and bidirectional collapse
- [STRIDE Mapping](../stride/) — systematic threat analysis across all six STRIDE categories
- [Guidance Gap Analysis](../guidance-gap/) — where current frameworks fall short
- [ACF Taxonomy](../../acf/) — the core failure modes with risk ratings and detection status
- [Glossary](../../reference/glossary/) — complete definitions for all terms
