...
**Why this tooling does not yet exist.** The gap is not an oversight. Semantic correctness — understanding what code *should do* in its institutional context — has traditionally been carried by the human author. Tools were built to check syntax, structure, and known defect patterns, not whether a system implements the right institutional meaning. Until now, a human author was usually in the loop at the point of creation as well as review, supplying the context that determined whether a default was helpful or catastrophic.
...
The tooling around this limitation is evolving quickly — **persistent memory stores, project-rule files, larger context windows, and checkpointing features** are arriving faster than governance cycles can track, and some context-collapse failures may attenuate as they mature. The structural argument nevertheless survives. Memory stores and rule files change *where the rule lives*, not what kind of thing it is: an explicit declaration still competes for salience against training priors rather than becoming internalised judgment. More importantly, **training-distribution bias is untouched by session-level memory** and may deepen as agent-generated code enters future training corpora. Controls should therefore target the underlying condition — **context-bounded generation from biased priors** — rather than the symptom rate of a particular tool generation.
...
Model diversity is the obvious mitigation to monoculture, but it should not be assumed to provide independence. Even where organisations use different models, overlapping training distributions and shared lineages may preserve correlated tendencies. A further possibility — not yet empirically demonstrated — is that distinct defects from different models could compose: Model A generates a data-ingestion function that omits the validation boundary between external and internal data ([ACF-T1](../../acf/t1-authority-tier-conflation/)); Model B, working on downstream processing, encounters the now-unvalidated data and adds a `.get()` default to handle its potential absence gracefully ([ACF-S1](../../acf/s1-fabricated-default/)). Neither model produced the other's defect, but Model B's "fix" converts Model A's loud failure (a crash on missing validation) into a silent one (a fabricated default on unvalidated data). The composed outcome would be worse than either defect alone.
...
- [Trust Boundaries](../trust-boundaries/) — the authority-tier model that defensive patterns collapse
- [STRIDE Applied to Agentic Code](../stride/) — the failure modes classified by threat category
- [ACF Taxonomy](../../acf/) — the catalogue of observed failure modes
