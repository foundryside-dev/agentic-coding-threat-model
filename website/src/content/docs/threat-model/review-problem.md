...
The consequences of this asymmetry are already visible at scale — GitHub's platform-level PR restrictions, discussed in [the introduction and scope](../introduction/), are a direct institutional response to the volume assumption breaking down.

The dynamics have a close parallel in diagnostic imaging: AI-assisted throughput increases output without adding patient-specific clinical context, shifts the reviewer from active diagnosis to passive scanning under volume pressure, and extends the practice into settings where the original governance assumptions no longer hold.

This asymmetry matters most for changes known to be agent-generated via provenance tracking ([the response controls](../response-landscape/)). Semantic boundary enforcement, however, should apply uniformly regardless of authorship — authorship-based differential enforcement is unsolvable in mixed workflows where agent-generated and human-authored code are interleaved.
...
The incident in [the case study](../../respond/case-study/) suggests that even under favourable conditions — a context-loaded operator using the agent itself as an investigative instrument — surfacing a semantic defect concealed by conventional-looking code required sustained multi-step probing.

Case study observation ([the case study](../../respond/case-study/)) confirms the mechanism in compliance-constrained environments: agent-generated code containing semantic defects — trust boundary violations, defensive patterns on audit-critical data, missing validation boundaries — has entered codebases after passing human review processes designed to catch exactly these issues, with defects subsequently identified through automated enforcement tooling, adjacent code review, or downstream test failures.
...
- Project-level instructions and memory stores can encode rules ([the response controls](../response-landscape/)), but that knowledge competes for finite context and remains advisory rather than enforceable
...
Organisations deploying semantic enforcement should define a tool-precedence hierarchy that explicitly prioritises semantic enforcement over conventional linting where they conflict, and should require human review of agent-authored allowlist entries. Standard linters encode community conventions while semantic enforcers encode institutional knowledge; when they conflict, the institutional knowledge should win. A machine-readable precedence declaration can help an agent resolve that conflict correctly.

The longitudinal [case study](../../respond/case-study/) has since hardened this requirement into a structural control: agents can propose exceptions but cannot enact them. An exception takes effect only when a human applies a cryptographic signature the agent does not hold, with automated triage filtering proposals before they reach the human.

## See also

- [The Response Landscape](../response-landscape/) — the practical response: check differently rather than review harder
- [ACF-D1: Finding Flood](../../acf/d1-finding-flood/) and [ACF-D2: Review Capacity Exhaustion](../../acf/d2-review-capacity-exhaustion/) — denial-of-service failure modes that exploit review limitations
- [STRIDE Applied to Agentic Code](../stride/) — the failure modes classified by threat category
- [Bibliography](../../reference/bibliography/) — sources for the automation-bias, productivity, defect-rate, and systems-archetype evidence
