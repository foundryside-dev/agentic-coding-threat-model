...
The distinct failure for an ungranted project-local pack is equally explicit:
...
:::note
**Release status.** Those messages are current-branch text. Released v1.5.0 emits shorter forms because the `wardline mcp` launch-flag route is itself branch-only. The fail-closed behaviour is identical in both.
:::

**The consequence:** a repository can *ask* for arbitrary trust semantics. It cannot *obtain* them. The gate that decides is on the caller's side of the boundary, and the failure mode when the two disagree is a hard exit, not a silently weaker policy. `wardline install <pack>` only emits guidance to add a pack to `weft.toml`; it never writes the file on the operator's behalf.
...
|---|---|---|
| `@external_boundary` | none | An external entry point. Its return value carries <State name="EXTERNAL_RAW" />. |
...
A pack is the extension point: an importable module that may carry a `config` attribute (deep-merged over the project's `[wardline]` table) and may register additional rules on the same config-gated path as the builtins. Packs are how an organisation encodes vocabulary its own domain needs without a fork.
...
:::note
**Release status.** The grant-residency machinery in this section — `wardline mcp --trust-pack` / `--allow-custom-packs` as launch flags, preserving grant arguments across installer and doctor repair runs, and reading grants back for diagnosis — is present on the `release/1.5.0` branch and not in the v1.5.0 release. Released v1.5.0 does ship the CLI grants on scan-family commands and per-call MCP tool arguments.
:::
...
A repository that also controls the MCP launch entry an agent host reads is a different, and weaker, position — one that belongs to whoever provisions the agent's tooling. Recorded as [residual risk 8](../residual-risks/#trust-grant-and-suppression-risks).
:::

## No manifest is required

The designed specification defined a root `wardline.yaml` with tier definitions, boundary declarations, dependency taint entries, and rule overrides — a substantial schema, and the artefact the entire governance model was built around. **It was never built.** The file with that name in wardline's own repository is 101 bytes of federation URLs and has nothing to do with the designed schema.
...
- [Python Reference](../python-reference/) — the full `weft.toml` key set and command surface
- [Gates, Suppression, and the Judge](../gates-suppression-and-judge/) — the same principle applied to the gate
- [Language Frontends](../language-frontends/) — where a granted pack becomes analysis behaviour
