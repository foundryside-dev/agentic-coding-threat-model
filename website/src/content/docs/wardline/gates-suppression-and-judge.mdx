...
The [lattice](../trust-lattice/) assigns trust, the [declarations](../declarations-and-trust-grants/) fix it, the [rules](../rules/) make it checkable. This page is the last step: turning a stream of findings into a decision a pipeline can act on, and what may and may not overturn that decision.
...
This is the parent paper's [seventh verification property](../verification/#property-7--inertness-detection), implemented.
...
- The **honesty gap** is the `unknown` set: entities whose trust could not be proven, because there is no computed return taint, because the tier is undeclared or an unknown state such as <State name="UNKNOWN_RAW" />, or because the engine under-scanned. Files discovered but never analysed each count as at least one uncovered surface item.
...
- [Verification Properties](../verification/) — the seven properties, and inertness as property 7
- [Residual Risks](../residual-risks/#trust-grant-and-suppression-risks) — grant residency, unsigned suppression, judged drift, attestation limits
- [Python Reference](../python-reference/#suppression-file-formats) — the three file formats
