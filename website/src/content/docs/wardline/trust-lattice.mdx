...
import LatticeTable from '../../../components/LatticeTable.astro';
import LatticeDiagram from '../../../components/LatticeDiagram.astro';

The lattice is the foundation of everything else. A [declaration](../declarations-and-trust-grants/) assigns a state in it; a [rule](../rules/) reads a state out of it and compares two of them; the severity model modulates by it; the [gate](../gates-suppression-and-judge/) fires on the result.
...
`TRUST_RANK` totally orders them from most trusted, <State name="INTEGRAL" />, at rank 0 to least trusted, <State name="MIXED_RAW" />, at rank 7. Values are explicit uppercase strings so that serialised findings, cache keys, and fixtures stay stable across releases.
...
`RAW_ZONE` — `{EXTERNAL_RAW, UNKNOWN_RAW, MIXED_RAW}` — is the single source of truth for the raw-tier gates in <Rule id="PY-WL-101" />, `106`, `107`, `108`, and `109`, so a future raw-zone state cannot drift between rule modules.
...
- [Trust Boundaries](../../threat-model/trust-boundaries/) — the parent paper's authority tier model
- [Rules](../rules/) — how the lattice modulates severity
- [Roadmap: The Unbuilt](../roadmap-the-unbuilt/) — trusted restoration boundaries, the missing producer for two states
