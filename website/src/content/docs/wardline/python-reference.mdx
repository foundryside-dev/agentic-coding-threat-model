...
Levels may be passed as the string name or as a `wardline.core.taints.TaintState` member. An external boundary returns <State name="EXTERNAL_RAW" />. An unknown name, or a level outside the permitted set for that decorator, raises `ValueError` **at decoration time**; the static equivalent is <Rule id="PY-WL-114" />.
...
The effect runs in **both directions and improves neither**: `MIXED_RAW` sits in the freedom zone of the severity model, so findings *inside* a function that resolves to it are suppressed; and <Rule id="PY-WL-101" /> fires on it as the *actual return* of a trusted producer — exactly the false positive on correct code that took the operator off the default path in the first place. [Full account](../trust-lattice/#combination-two-operators-one-in-use).
...
The four grant flags are the [caller-side controls](../declarations-and-trust-grants/#the-pattern-generalises-five-grants); the two `--fail-on-*` sub-gates and the `NOT_EVALUATED` verdict belong to [the gate](../gates-suppression-and-judge/#the-gate-decision).
...
`attest` also accepts `--allow-dirty` (records `dirty: true`), `--cache-dir`, `--loomweave-url`, the pack grants, and `--strict-defaults`. `assure` takes only `--config` and `--format`. [Signing, key handling, and the limits of HMAC-under-a-shared-key](../gates-suppression-and-judge/#wardline-attest--a-signed-posture-bundle).
...
:::caution
**Grant-residency caveat:** pack grants placed in the `.mcp.json` entry's `args` array are **preserved across re-runs**. See [residual risk 8](../residual-risks/#8-grant-residency).
:::
...
`wardline findings [PATH]` is the read-only filtered view: `--rule-id`, `--severity`, `--sink`, or an arbitrary JSON `--where` filter, printed as JSONL. For example:

```bash
wardline findings --where '{"rule_id":"PY-WL-106"}'
```
...
**Now consider the half-fix:** a boundary that is declared but does not reject — `def parse_order(raw): return json.loads(raw)` and nothing else. <Rule id="PY-WL-101" /> clears, because the declared and actual levels now agree. <Rule id="PY-WL-102" /> fires in its place:
...
- [Declarations and Trust Grants](../declarations-and-trust-grants/) — what the grant flags do
- [Gates, Suppression, and the Judge](../gates-suppression-and-judge/) — the gate decision model
- [Rules](../rules/) — the 28-rule catalogue
