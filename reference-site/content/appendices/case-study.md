...
### Postscript: the validation boundary as built

*Added in v0.2.0. This describes the longitudinal project's enforcement machinery approximately five months after the observations above were captured. It is included because the machinery now operationalises several of this paper's proposals as an executable CI pipeline, and because the failures encountered while building it are themselves evidence. The account is de-identified; the mechanisms, not the project, are the point.*

At the time of the earlier observations the enforcement regime was a single AST pattern-matching gate with a YAML allowlist. It has since grown into a **four-layer validation boundary**, each layer enforcing *meaning* at a different surface.
...
**Layer 1 — what elevates it from linter to control.** Two properties of the gate's *invocation*. First, the **caller** grants the trust pack: the scanner refuses a repository's custom trust declarations unless the invoking pipeline explicitly authorises them, so repository configuration cannot quietly vouch for its own trust story. Second, the invocation enables **inertness detection** — a scan that recognised zero declared boundaries exits red rather than green, making "checked and clean" structurally distinguishable from "didn't look" ([verification property 7]({{< relref "/threat-model/response-landscape" >}}#minimum-verification-properties)). The project pins that invocation, grant flags and inertness flag included, with a test over its own agent-instructions file, so the gate's posture cannot drift silently. The scanner is the tool documented in the [wardline section]({{< relref "/wardline" >}}); its trust-grant and inertness mechanics are described there.

**Layer 2 — signed exceptions, and whose machinery they are.** The tier-model rule has a production incident behind it: a guard built on structural typing — a runtime-checkable protocol — passed impostor objects while rejecting the real vendor object, the [ACF-S3]({{< relref "/acf/s3-structural-identity-spoofing" >}}) failure shape encountered live, with approximately 33,000 unit tests green and one live HTTP call finding it. In this layer — **the project's own lint suite, not the taint scanner** — every suppression is a judged exception carrying owner, mandatory reason, and expiry, sealed with an HMAC signature whose key only the human operator holds. Agents stage exception candidates key-free; only the operator can sign; and the signing step re-derives each finding from the live tree, so a staged claim that no longer matches reality fails at signature time.

> [!NOTE]
> **The two layers have different governance postures, deliberately.** The owner/expiry/signature regime described above belongs to the project's repository-local lint suite. The taint scanner's own suppression stores carry no owner, no reviewer identity, and no signature — its answer to the same threat is mechanical rather than procedural, and is set out in [the "No governance" position]({{< relref "/wardline/gates-suppression-and-judge" >}}#the-no-governance-position). Reading either regime onto the other misstates both.

An exception in the signed regime therefore carries provenance and cannot be minted by the code — or the agent — asking for it. This is the structural answer to the incident in the annotated transcript below (E.1–E.3), where an agent resolved a tool conflict by writing itself a permanent allowlist entry: that pathway no longer exists. It also removes the self-authorisation route by which a remediation "fix" can silence the finding it was meant to resolve ([ACF-R5]({{< relref "/acf/r5-remediation-induced-violation" >}})).

**The judge in the loop.** Between the agent's proposal and the operator's signature sits an automated judge: a prompted model evaluates each flagged finding against the declared rule and produces a per-finding accept/block verdict with a bounded, prescribed-form rationale. **The judge signs nothing.** The signing tool re-runs the judge on every substantive entry it is asked to sign — re-judging at signature time is what prevents laundering a stale verdict over code that has since changed — and signs only what the judge accepts. A blocked verdict cannot enter the exception register through the normal path at all; the operator can override, but the override is recorded alongside the model's actual verdict, and the override rate is itself a tracked governance signal.

The division of labour is deliberate: **the judge absorbs the finding flood ([ACF-D1]({{< relref "/acf/d1-finding-flood" >}})) so that human attention lands only on ambiguous or high-stakes exceptions — before habituation converts exception review into rubber-stamping — while the human, holding the key, remains the sole authority.** Judging a *specific flagged violation against a specific declared rule* is a far narrower task than not producing the pattern in the first place, which is why a model that reliably generates ACF-S1 can still usefully triage it. Agent self-checking is a load-bearing *filter* and an unacceptable *authority*.

**The honest ledger.** Two of these layers exist because the gates' own operators were fooled first, and the incidents are as instructive as the machinery.

1. **The vacuous green.** The tier-model lint command had run in CI for months in a form that **loaded zero rules and exited success** — a green that would have certified any tree. Two successive acceptance rounds were signed off against that gate before the hole was found; the project's own review notes observe that it had reproduced, in its second gate, exactly the inertness hole its first gate had been built to close. The first real run produced a baseline of several hundred findings — a baseline, not a regression — and the incident motivated inertness detection as a named verification property.
2. **The deliberately red gate.** At the time of writing, the tier-model gate exits non-zero with a substantial corpus of findings awaiting judged signature. That is the designed posture — unauthorised merges stay blocked while the signing debt stays visible — rather than a green gate purchased by weakening the rules.

**The defence is not that the codebase is clean; it is not.** The defence is that every gate fails closed, distinguishes "checked and clean" from "didn't look," and requires a recorded human decision to overrule — which is what separates a codebase full of *known* semantic findings from one full of unknown ones.

**The pipeline as a whole.** Agent generates code → the semantic gate fails → the agent proposes an exception with rationale → the judge triages the proposal → the operator signs, or refuses, with a key the agent never holds. Each element answers a threat this paper names: inertness detection answers vacuous assurance; caller-granted trust packs answer configuration self-authorisation; signed exceptions answer allowlist-broadening and ACF-R5; judge triage answers ACF-D1/D2; output oracles answer semantic drift; dialect-parity invariants answer the data-layer permissiveness gap. It also demonstrates the asymmetry in sustained operation: **the gates are stricter than most teams would impose on human developers, and the agents simply iterate until they pass.**

**Caveats.** The same single-project limitations apply: one codebase, one operator, machinery too young for outcome statistics. The gate state is red by design, and the signing debt is real. This account is offered as evidence that the proposed control class is implementable — and of the failure modes an implementer should expect on the way — not as evidence that the approach is sufficient.
