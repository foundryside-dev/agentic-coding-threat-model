...
weight: 2
acf_tags: ["ACF-S1", "ACF-T1", "ACF-R1", "ACF-R2"]
---
...
The [autonomy self-assessment]({{< relref "/appendices/autonomy-assessment" >}}) could inform governance design for SQL-producing populations, though its current framing targets developer workflows and a separate practitioner-facing artefact would be required for non-developer populations.
...
The scheduled query that runs every night and has run without incident for three years is the SQL equivalent of the legacy system whose implicit security properties are paved over by modernisation: nobody remembers why it works, and nobody will notice when it starts producing wrong results.

## See also

- [ACF-S1: Fabricated Default]({{< relref "/acf/s1-competence-spoofing" >}}) — COALESCE as default-value fabrication in SQL
- [ACF-T1: Authority Tier Conflation]({{< relref "/acf/t1-authority-tier-conflation" >}}) — INSERT...SELECT crossing authority tiers without validation
- [ACF-R1: Audit Trail Destruction]({{< relref "/acf/r1-audit-trail-destruction" >}}) — UPDATE/DELETE that overwrites without preserving prior state
- [ACF-R2: Partial Completion]({{< relref "/acf/r2-partial-completion" >}}) — multi-statement operations without explicit transactions
