...
**Why no one built it.** The gap is not an oversight. Semantic correctness — understanding what the code *should do* in its institutional context — was the programmer's core professional contribution. No one builds a tool to check whether an architect designed a hospital instead of an office, because understanding the client's requirements is what the architect is *for*. Until now, a human author was always in the loop at the point of *creation*, not just at the point of review, and that author carried the institutional context that determined whether a default was helpful or catastrophic.
