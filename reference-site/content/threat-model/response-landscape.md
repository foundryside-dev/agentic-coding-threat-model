...
Everything in this section applies to a *scoped subset* of the codebase, and the scoping decision is itself the first control. In most systems the overwhelming majority of code is not high-stakes: UI rendering, presentation formatting, service-continuity logic, and ordinary CRUD paths correctly use defensive programming, and applying fail-fast rules to them produces brittle software, developer friction, and a finding flood ([ACF-D1]({{< relref "/acf/d1-finding-flood" >}})) that discredits the enforcement layer.

**A risk-averse reading of this threat model that applies semantic enforcement codebase-wide would be a worse outcome than not applying it at all.** The organisation must name its high-stakes paths before enforcement can begin.

Three converging tests. A path is in scope if it passes **any** of them:

| Test | Question |
|------|----------|
| **The data test** | Does the path read, write, or move data whose authority tier is Tier 1 or Tier 2 — audit records, security classifications, entitlements, clinical or financial or evidentiary records — or carry data across a tier boundary (external ingest, partner API consumption, deserialisation of stored artefacts)? |
| **The consequence test** | If this code silently produced a wrong value, would the error propagate into a decision, a record of authority, or a downstream system before any human noticed? The review judgement question ("if this code is wrong, how would I find out?") applied at scoping time rather than review time. |
...
**The false-positive economics differ by who absorbs the finding.** Twenty years of SAST deployment practice has been shaped by *human* tolerance: rules are weakened, precision is prioritised over recall, and findings queues are tuned to what developers will accept before they disable the tool. That calculus changes when the author absorbing the finding is an agent. An agent confronted with a red gate does not fatigue, resent the rule, or lobby to have it removed — it iterates on the code until the gate passes, at machine speed, as many times as required.

Organisations can therefore afford **stricter and less precise rules on agent-authored changes** than they would ever impose on human developers: recall can be prioritised over precision, because the cost of a false positive is agent iterations rather than human goodwill.

The constraint relocates rather than disappearing. Three limits:

- The **human-facing exception path must stay scarce and adjudicated** — a gate whose exceptions are cheap is not a gate.
- **Imprecise rules still teach the agent to satisfy the rule's letter rather than its intent** — the rules always trail the failure modes.
- **The humans who adjudicate escalated exceptions are still subject to habituation**, which is the motivation for triaging proposed exceptions before they reach a human decision.
...
Any enforcement tool proposed for high-assurance environments should be assessed against seven verification properties:
...
7. **Inertness detection (fail-closed on an empty scan).** The gate must distinguish "checked and clean" from "didn't look." A scan that loaded zero rules, recognised zero declared boundaries, or matched zero files **must fail, not pass** — because a green result from an inert gate is worse than no gate at all: it converts the absence of checking into apparent assurance, and every layer of governance above it (review posture, control law status, accreditation evidence) inherits the false signal. This failure mode is not hypothetical: the longitudinal case study's enforcement pipeline ran one of its lint gates with **zero rules loaded, exiting green, for months** before the misconfiguration was noticed. Inertness detection is the gate-level analogue of the mutation-tested test suite — it verifies that the verifier is actually verifying.

These are evaluation criteria, not product features. Any organisation building, procuring, or extending enforcement tooling should verify these properties. The criteria are tool-agnostic. The [wardline as-built specification]({{< relref "/wardline/verification" >}}) works through all seven against one shipped implementation, including the two it does not satisfy.
...
This points to a human limitation distinct from time pressure: *cognitive range*. A senior backend engineer cannot practically adopt a security architect's analytical frame, or a data quality auditor's — not because they lack intelligence, but because genuine expertise in each frame takes years to develop. Prompted agents do not have deeper expertise in any single frame than a domain specialist, but they can adopt multiple frames without the switching cost that makes it impractical for any individual human reviewer to cover more than one or two perspectives well. In this sense, prompted agents appear to function as *polymorphic* reviewers — a breadth of analytical coverage that is impractical for an individual reviewer. This characterisation is based on the author's observation in a single project context; broader empirical validation remains an open question.
...
The correlated failure property cuts both ways. Fifty instances of the same defensive anti-pattern across a codebase are **one systematic defect, not fifty independent bugs** — and once the pattern is encoded as a detection rule, remediation cost decouples from instance count.

The lifecycle, observed in production:

1. A human recognises the failure shape and understands why it is wrong
2. The understanding is encoded as a detection rule precise enough to act on
3. The rule sweeps the codebase and finds every instance
4. Agents remediate the instances in parallel, with walltime largely independent of file count
5. The rule joins the CI gate and prevents recurrence

**Steps 1–2 are irreducibly human** — the scarce resource is the detection rule, not the ability to push fixes. Steps 3–5 are where agent velocity becomes an asset rather than a liability.

Two caveats keep this control honest:

- **Remediation commits require equal or higher review scrutiny than new code, not less.** [ACF-R5]({{< relref "/acf/r5-remediation-induced-violation" >}}) documents agents introducing a *different* violation inside the fix itself, precisely where reviewer attention is anchored on the original finding. The rule re-run plus the existing test suite verifies that the flagged pattern is gone; it does not verify that the replacement preserves every property the original code had.
- **The control depends on provenance tracking for efficient targeting.** With provenance, the sweep can prioritise agent-generated regions; without it, the sweep must cover everything.

### Baseline-and-ratchet enforcement for unpatternable rules

Pattern-wide remediation presupposes that the failure shape can be encoded as a detection rule. Some invariants have **trust domain, not syntax, as their subject** — the same tokens are mandatory at one boundary and forbidden one function up — so any syntactic rule carries both false positives and false negatives *by construction*.

For those invariants, a different enforcement model applies: **freeze the corpus instead of matching the pattern.**

- Enumerate every current site of the construct
- Grant narrow structural amnesties for provably safe shapes
- Require a named adjudication entry for everything else
- **Fail the gate on drift in any direction** — new sites, vanished entries, or changed occurrence counts

The machine then enforces the property it *can* check ("no unadjudicated change"), while humans adjudicate the property it cannot (whether each site is semantically legitimate), site by site, over time.
...
The pattern is the fingerprint-baseline concept generalised from an annotation surface to an entire defect class — and it is deliberately proportionate: no signatures, no authority-tier coupling, no new tooling category. **Not every semantic control needs the full enforcement stack.**

Three further properties emerged from the working example's first day of operation:

1. **The baseline doubles as a scan-completeness oracle.** Because a vanished entry is an error, a truncated or partial scan cannot produce a false green — every site the scan failed to reach surfaces as a stale entry, so the gate fails closed under conditions (resource contention, interrupted tree walks) that per-root coverage assertions do not detect.
2. **Prefer a hole that can be enumerated over one that must be trusted.** A structural amnesty widened to recognise nested variants creates a permanently invisible class; a handful of named entries costs nothing and stays countable.
3. **The adjudication ledger is itself authoritative (Tier 1) data and must be parsed accordingly.** It is maintained by hand, and a lenient loader that skipped a malformed entry would silently drop sites from the ledger — indistinguishable downstream from sites legitimately cleared — so the loader must fail loudly, with position information, on any malformation. *That is the fail-fast-on-integrity rule the enforcement regime exists to impose, applied to the regime's own artefact.*
...
Step 4 is the learning step that closes the loop. Without it, the next correlated defect pattern will enter through the same gap. The execution mechanics of steps 1–3 at scale — parallel agent remediation once the rule exists — are formalised as the **automated pattern-wide remediation** control above.
